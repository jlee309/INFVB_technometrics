...
rm(list=ls())
getwd()
#+#(note) We need to check W, because this model does not have W. 
...
# load("../01DataGeneration/G_phi1_Gen500_1.RData")
################################################################################
...
comptTime_INLA<-res$cpu.used/60 # Computation Time (in minutes)
...
save(obs_RMSPE_INLA  ,comptTime_INLA,
