...
################################################################################
# 11142024
# nsim_SA_INFVBPhi_Gaussian Data
################################################################################
################################################################################
rm(list=ls())
library(mvtnorm);
...
# By using Multivariate Normal for v(Beta)
...
for(phiselect in 1:length(phiset) ){
  ...
  # for (sim in 1:n_simulations) {
    for (sim in 15:n_simulations) {
    cat("Running simulation", sim, "\n")
    file_name1 <- paste0("../01DataGeneration/G_phi", phiselect, "_Gen500_", sim, ".RData")
    load(file_name1)
    file_name2 <- paste0("../03INFVBfixphi_ParallelComputing/G_phi", phiselect, "_Gen500_", sim, "_VBphiParallel.RData")
    load(file_name2)
...
################################################################################
# (1) Getting 50 samples for V(Beta) and ssq
numofsamples <- 50
sample_V_list <- list()
...
for(i in 1:iter.thetaphi){
  ...
  for(j in 1:numofsamples){
    ...
    sample_eta_testdata_VB[[i]][[j]]<-X_testdata %*%sample_V_list[[i]][j,][c(1:p)]+Sigma_21%*%inv_Sigma_11%*%(Z_trainingdata-X_trainingdata%*%sample_V_list[[i]][j,][c(1:p)])
...
save(eta_trainingdata_real, eta_testdata_real,
     ...
     mu_sample_pred_VB,
     file = paste0("SA_G_phi",phiselect,"_Gen500_",sim,"_Full_INFVBphi.RData"))
  }
}
