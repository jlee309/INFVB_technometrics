...
################################################################################
# 10172024 
# nsim_Sampling method for INFVBphissq
# Only for when phi=0.7 (Divide this because of computing time)
rm(list=ls())
library(mvtnorm);
...
for (sim in 1:n_simulations) {
  ...
  load(file_name)
  file_name <- paste0("../04INFVBfixssqphi_ParallelComputing/B_phi", phiselect, "_Gen500_", sim, "_VBphissqphiParallel.RData")
  load(file_name)
  ...
  ################################################################################
  # (1) Getting 50 samples for V(Beta, W) 
  numofsamples<-50
  sample_V_list <- list()
  ...
  for (k in 1:(iter.thetaphissqphi)^2) {
    ...
    thetaC_covMatVssqphi_Mat<-diag(thetaC_covMatVssqphi[k,])
    
    # Draw samples
    sample_V<-rmvnorm(numofsamples, mean=thetaC_MeanVssqphi[k,], sigma= thetaC_covMatVssqphi_Mat)
  ...
  for(i in 1:(iter.thetaphissqphi)^2){
    ...
    Sigma_11 <- ELBOtables[i,1] * exp(-distMat[c(1:(0.8 * n)), c(1:(0.8 * n))] / ELBOtables[i,2])
    inv_Sigma_11 <- solve(Sigma_11)
    Sigma_21 <- ELBOtables[i,1] * exp(-distMat[c((0.8 * n + 1):n), c(1:(0.8 * n))] / ELBOtables[i,2])
    Sigma_22 <- ELBOtables[i,1] * exp(-distMat[c((0.8 * n + 1):n), c((0.8 * n + 1):n)] / ELBOtables[i,2])
    Sigma_12 <- ELBOtables[i,1] * exp(-distMat[c(1:(0.8 * n)), c((0.8 * n + 1):n)] / ELBOtables[i,2])
    ...
    for(j in 1:numofsamples){
      ...
      # sample_CovMat_prediction_VB[[i]][[j]] <- as.numeric(sample_CovMat_prediction_VB_ing) #don't need anymore
      sample_W_test_list[[i]]<-rmvnorm(n=numofsamples, mean=Sigma_21%*%(inv_Sigma_11%*%sample_V_list[[i]][j,][c((p+1):(0.8 * n+p))]), sigma =sample_CovMat_prediction_VB_ing)
  ...
  sample_W_test_list[[10000]]
  sample_eta_trainingdata_VB[[1]][1]
  sample_eta_testdata_VB[[1]][1]
  
  rm(sample_V_list,sample_W_test_list)
  ...
  #just for checking 
  ELBOmean_list_est[[2]]
  ELBOmean_list_est[[10000]]
  length(ELBOmean_list_est[[2]])
  
  ################################################################################
  # (4) Making the sample covariance matrix for the estimation 
  ################################################################################
  
  cov_sample_eta_trainingdata_VBssqphi_all<-matrix(data=rep(0,(0.8*n)^2), nrow=(0.8*n), ncol=(0.8*n))
  for(i in 1:(iter.thetaphissqphi)^2){
    if(i%%100 == 0) {print(i)}
    ...
    cov_sample_eta_trainingdata_VBssqphi_all<-cov_sample_eta_trainingdata_VBssqphi_all+cov_sample_eta_trainingdata_VB_i
    #-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-Delete for the memory#-#-#-#-#-#-#-#-#-#-#-#-#-#
    rm(list = dynamic_name)
  }
  
  #Just for checking
  dim(cov_sample_eta_trainingdata_VBssqphi_all)
  ...
  ################################################################################
  mu_sample_est_VBssqphi<-Reduce("+",ELBOmean_list_est)
  mu_sample_est_VBssqphi
  ...
  #just for checking
  ELBOmean_list_pred[[2]]
  
  ################################################################################
  # (4-1) Making the sample covariance matrix for the estimation 
  ################################################################################
  
  cov_sample_eta_testdata_VBssqphi_all<-matrix(data=rep(0,(0.2*n)^2), nrow=(0.2*n), ncol=(0.2*n))
  for(i in 1:(iter.thetaphissqphi)^2){
    ...
    cov_sample_eta_testdata_VBssqphi_all<-cov_sample_eta_testdata_VBssqphi_all+cov_sample_eta_testdata_VB_i
    #-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-Delete for the memory#-#-#-#-#-#-#-#-#-#-#-#-#-#
    rm(list = dynamic_name)
  }
  
  #Just for checking
  dim(cov_sample_eta_testdata_VBssqphi_all)
  rm(sample_eta_testdata_VB)
  ################################################################################
  # Making mu_sample_pred_VB
  ################################################################################
  mu_sample_pred_VBssqphi<-Reduce("+",ELBOmean_list_pred)
  mu_sample_pred_VBssqphi
  rm(ELBOmean_list_pred)
  
  save(eta_trainingdata_real, eta_testdata_real,
       cov_sample_eta_trainingdata_VBssqphi_all, cov_sample_eta_testdata_VBssqphi_all,
       mu_sample_est_VBssqphi,mu_sample_pred_VBssqphi,
       file = paste0("SA_B_phi",phiselect,"_Gen500_",sim,"_Full_INFVBssqphi.RData"))
}
# }