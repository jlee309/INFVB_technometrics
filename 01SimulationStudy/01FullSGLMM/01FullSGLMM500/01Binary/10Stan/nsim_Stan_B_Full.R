...
################################################################################
# 11092024 
# Full SGLMM Stan file for Binary Data
################################################################################
################################################################################
rm(list=ls())
...
dim(distMat_M)
gamma_samples<-stan(file="FullSGLMM_B.stan", 
                    data = list(N=nn,
                                K=k,
                                P=nn,
                                y=obs_trainingdata,
...
exp_prediction_Stan<-matrix(rep(NA,(0.2*n)*lengthPredSamples), nrow=(0.2*n), ncol=lengthPredSamples)
dim(exp_prediction_Stan)
...
for(i in 1:lengthPredSamples){
  ...
  exp_prediction_Stan[,i] <- exp(eta_testdata_MCMC_Stan[,i])/(1+exp(eta_testdata_MCMC_Stan[,i]))
  # Prediction 
  CovMat_prediction_MCMC_Stan[,i]<-as.numeric(Sigma_22-Sigma_21%*%inv_Sigma_11%*%Sigma_12)
}
exp_prediction_Stan<-apply(exp_prediction_Stan,1,mean) 
obs_RMSPE_MCMC_Stan<-sqrt(mean((exp_prediction_Stan-obs_testdata)^2))
obs_RMSPE_MCMC_Stan
rocBinary <- roc(obs_testdata, exp_prediction_Stan)
# Get the full AUC
aucVal_Stan <- auc(rocBinary)
aucVal_Stan
save(summaryMat,parMat,omegasMat,totTime,
     obs_RMSPE_MCMC_Stan,exp_prediction_Stan,aucVal_Stan,
     eta_testdata_MCMC_Stan, eta_trainingdata_MCMC_Stan,CovMat_prediction_MCMC_Stan,
    file = paste0("B_phi",phiselect,"_Gen500_",sim,"_MCMC_Stan.RData"))
}}
