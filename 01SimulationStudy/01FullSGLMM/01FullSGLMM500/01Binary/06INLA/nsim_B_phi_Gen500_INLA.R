...
exp_prediction_INLA<-exp(field_pred_mean)/(1+exp(field_pred_mean)) #Prediction using logit link
obs_RMSPE_INLA<-sqrt(mean((obs_testdata-exp_prediction_INLA)^2)) #CVRMSPE to be used as gold standard
comptTime_INLA<-res$cpu.used # Computation Time 
rocBinary <- roc(obs_testdata, exp_prediction_INLA)
...
save(exp_prediction_INLA , obs_RMSPE_INLA , aucVal_INLA ,comptTime_INLA,
     ...
     file = paste0("B_phi",phiselect,"_Gen500_",sim,"_INLA.RData"))
  }
}
