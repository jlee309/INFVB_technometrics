...
###########################################################################################
# 11122024 
# CRPS Measure for the count_nsim
rm(list=ls())
library(scoringRules)
getwd()
# setwd("/scratch/jlee309/01VBpaper/01SimulationStudy/01FullSGLMM/01FullSGLMM500/02Count/07Measures")
getwd()
################################################################################
################################################################################
...
logsTotal_VBssqphi_median_vector<-c(logsTotal_VBssqphi_median_vector,logsTotal_VBssqphi_median)
  ...
  save(crpsTotal_MCMC_mean_vector, crpsTotal_INLA_mean_vector, 
       ...
       file = paste0("C_phi",phiselect,"_5_Gen500_CRPS.RData"))
}

# rm(list=ls())
# load("C_phi2_Gen500_CRPS.RData")
# mean(crpsTotal_MCMC_mean_vector)
# mean(crpsTotal_INLA_mean_vector)
# mean(crpsTotal_VBphi_mean_vector)
# mean(crpsTotal_VBssqphi_mean_vector)


# c1<-crpsTotal_MCMC_mean_vector
# c2<-crpsTotal_INLA_mean_vector 
# c3<-crpsTotal_VBphi_mean_vector
# c4<-crpsTotal_VBssqphi_mean_vector
# l1<-logsTotal_MCMC_median_vector
# l2<-logsTotal_INLA_meadian_vector 
# l3<-logsTotal_VBphi_median_vector 
# l4<-logsTotal_VBssqphi_median_vector 
# 
# length(c1)
# length(c2)
# length(c3)
...
# length(l4)
# 
# load("C_phi4_1to5_Gen500_CRPS.RData")
# 
# 
# crpsTotal_MCMC_mean_vector<-c(crpsTotal_MCMC_mean_vector, c1)
# crpsTotal_INLA_mean_vector<-c(crpsTotal_INLA_mean_vector, c2)
# crpsTotal_VBphi_mean_vector<-c(crpsTotal_VBphi_mean_vector, c3)
# crpsTotal_VBssqphi_mean_vector<-c(crpsTotal_VBssqphi_mean_vector, c4)
# 
# logsTotal_MCMC_median_vector<-c(logsTotal_MCMC_median_vector,l1)
# logsTotal_INLA_meadian_vector<-c(logsTotal_INLA_meadian_vector,l2)
# logsTotal_VBphi_median_vector<-c(logsTotal_VBphi_median_vector,l3)
# logsTotal_VBssqphi_median_vector<-c(logsTotal_VBssqphi_median_vector,l4)