...
################################################################################
# 10142024
# Count Data Generation when N=500
...
for(phiselect in 1:length(phiset) ){
  ...
  for (sim in 1:n_simulations) {
    ...
    lambda_trainingdata<-lambda[c(1:(0.8*n))]
    ...
    lambda_testdata<-lambda[c(((0.8*n)+1):(n))]
    w_testdata<-w[c((0.8*n+1):(n)),]
    ################################################################################
    ################################################################################
    save(phi,p,w,distMat,beta, sigma2, phi, n,
         X_trainingdata,locations_trainingdata,obs_trainingdata,w_trainingdata,
         R_phi_trainingdata,lambda_trainingdata,
         X_testdata, locations_testdata, obs_testdata, w_testdata,lambda_testdata,
         file=paste0("C_phi",phiselect,"_Gen500_",sim,".RData")) 
  } 
}
