...
################################################################################
# 11032024 
# nsim_Sampling method for INFVBphi
rm(list=ls())
...
################################################################################
n_simulations <- 5 #Number of simulations per each scenario
...
for (sim in 1:n_simulations) {
  ...
  file_name2 <- paste0("../03INFVBfixphi_ParallelComputing/C_phi", phiselect, "_Gen500_", sim, "_VBphiParallel.RData")
  load(file_name1)
  load(file_name2)
  ...
  for (k in 1:iter.thetaphi) {
    ...
    thetaC_covMatV_Mat<-diag(thetaC_covMatgamma[k,])
    
    # Draw samples
    sample_V<-rmvnorm(numofsamples, mean=thetaC_Meangamma[k,], sigma= thetaC_covMatV_Mat)
  ...
  save(eta_trainingdata_real, eta_testdata_real,
       ...
       file = paste0("SA_C_phi",phiselect,"_Gen500_",sim,"_Full_INFVBphi.RData"))
}
# }