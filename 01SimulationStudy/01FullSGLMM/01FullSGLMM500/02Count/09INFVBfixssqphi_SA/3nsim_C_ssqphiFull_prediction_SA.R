...
################################################################################
# 10142024 
# Sampling method for INFVBphissq
rm(list=ls())
...
for (sim in 1:n_simulations) {
  ...
  load(file_name)
  file_name <- paste0("../04INFVBfixssqphi_ParallelComputing/C_phi", phiselect, "_Gen500_", sim, "_VBphissqphiParallel.RData")
  load(file_name)
  ...
  ################################################################################
  # (1) Getting 100 samples for V(Beta, W) 
  numofsamples<-50
  sample_V_list <- list()
  ...
  for (k in 1:(iter.thetaphissqphi)^2) {
    ...
    thetaC_covMatVssqphi_Mat<-diag(thetaC_covMatgammassqphi[k,])
    
    # Draw samples
    sample_V<-rmvnorm(numofsamples, mean=thetaC_Meangammassqphi[k,], sigma= thetaC_covMatVssqphi_Mat)
  ...
  save(eta_trainingdata_real, eta_testdata_real,
       ...
       file = paste0("SA_C_phi",phiselect,"_Gen500_",sim,"_Full_INFVBssqphi.RData"))
}
# }