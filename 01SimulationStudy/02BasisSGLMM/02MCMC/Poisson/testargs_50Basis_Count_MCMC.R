################################################################################
################################################################################
# INFVB for Count Data using 50 Eigen Basis functions
################################################################################
################################################################################
rm(list=ls())
...
################################################################################
################################################################################
numofbasis<-50 # Choose the first 50 Eigen basis functions
...
mu_delta<-c(rep(0,numofbasis)) ;sigma_delta<-sigma2*diag(numofbasis) #sigma2?check
...
## Making vacant matrix for MCMC samples for (Beta1, Beta2, Simga2)
...
for ( i in 2:n.iteration){
  ...
  proposed_beta<-rnorm(n=2, mean=parameterMatrix[i-1,c(1,2)], sd= 0.02236068) #Proposal Distribution
  lambdaProp<-as.numeric(exp(X_trainingdata%*%proposed_beta+M%*%deltaMatrix[i-1,]))
  num<-(sum(dpois(x=obsPois_trainingdata,lambda=lambdaProp,log = TRUE)))-1/2*t(proposed_beta)%*%(inv_sigma_beta%*%proposed_beta)
  lambdaPrev<-as.numeric(exp(X_trainingdata%*%parameterMatrix[i-1,c(1,2)]+M%*%deltaMatrix[i-1,]))
  den<-(sum(dpois(x=obsPois_trainingdata,lambda=lambdaPrev, log = TRUE)))-1/2*t(parameterMatrix[i-1,c(1,2)])%*%(inv_sigma_beta%*%parameterMatrix[i-1,c(1,2)])
  alpha<-min(0,num-den)
  ...
  ##### I made changes here - All at once
    proposed_delta<-rnorm(n=numofbasis, mean=deltaMatrix[i-1,], sd=0.4)
    lambdaProp<-as.numeric(exp(X_trainingdata%*%parameterMatrix[i,c(1,2)]+M%*%proposed_delta))
    num<-(sum(dpois(x=obsPois_trainingdata,lambda=lambdaProp,log = TRUE)))-1/(2*parameterMatrix[i,3])*t(proposed_delta)%*%(inv_sigma_delta%*%(proposed_delta))
    lambdaPrev<-as.numeric(exp(X_trainingdata%*%parameterMatrix[i,c(1,2)]+M%*%deltaMatrix[i-1,]))
    den<-(sum(dpois(x=obsPois_trainingdata,lambda=lambdaPrev,log = TRUE)))-1/(2*parameterMatrix[i,3])*t(deltaMatrix[i-1,])%*%(inv_sigma_delta%*%(deltaMatrix[i-1,]))
...
for(i in 1:lengthPredSamples){
  ...
  lambda_prediction[,i] <-as.numeric(exp(eta_testdata_MCMC[,i]))
...
################################################################################
##Fist Method 
lambda_RMSPE<-sqrt(mean((lambda_prediction-lambda_testdata)^2))
lambda_RMSPE
################################################################################
################################################################################
## obs_RMSPE
obs_RMSPE<-sqrt(mean((lambda_prediction-obsPois_testdata)^2))
obs_RMSPE

save(parameterMatrix , deltaMatrix , ptFinal , 
     lambda_prediction, obs_RMSPE , lambda_RMSPE , n.iteration,
     acceptRate_marginal_delta , acceptRate_marginal_sigma2_delta , acceptRate_beta2 , 
     acceptRate_beta1, 
     eta_trainingdata_MCMC,eta_testdata_MCMC,
     file = paste0("50Basis_C_phi",phiselect,"_Gen25k_",sim,"_MCMC.RData"))