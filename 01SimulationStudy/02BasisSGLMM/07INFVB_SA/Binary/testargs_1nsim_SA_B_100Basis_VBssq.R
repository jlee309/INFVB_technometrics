...
################################################################################
# 11022024 
# Advanced Sampling Method for INFVB_100bases
rm(list=ls())
...
file_name3 <- paste0("../../03INFVB_ParallelComputing/Binary/100Basis_B_phi", phiselect, "_Gen25k_", sim, "_VBssqParallel.RData")
...
# (1) Getting samples for V(Beta, W)
numofsamples <- 50
sample_V_list <- lapply(1:iter.thetaphi, function(k) {
  if (k %% 100 == 0) print(k)
  thetaC_covMatV_Mat <- diag(thetaC_covMatV[k, ])
  rmvnorm(numofsamples, mean = thetaC_MeanV[k, ], sigma = thetaC_covMatV_Mat)
})
...
rm(list = setdiff(ls(), c("eta_trainingdata_real", "eta_testdata_real", 
                          "cov_sample_eta_trainingdata_VB_all", 
...
# Save only the required objects
save(cov_sample_eta_trainingdata_VB_all, cov_sample_eta_testdata_VB_all,
     mu_sample_est_VB, mu_sample_pred_VB,
     file = paste0("SA_100Basis_B_phi", phiselect, "_Gen25k_", sim, "_VBssqParallel.RData"))
#   }
# }
