...
for(phiselect in 1:length(phiset) ){
  ...
  for (sim in 1:n_simulations) {
    ...
    file_name5 <- paste0("../../04HybridMFVB/Binary/50Basis_B_phi", phiselect, "_Gen25k_", sim, "_HMFVB.RData")
    file_name6 <- paste0("../../06INLA/Binary/50Basis_B_phi", phiselect, "_Gen25k_", sim, "_INLA.RData")    
    load(file_name1)
    load(file_name2)
    load(file_name3)
    load(file_name4)
  ...
  save(AUC_MCMCvector, AUC_INLAvector, 
       ...
       file = paste0("50Basis_B_phi",phiselect,"_Gen25k_nsim50_Table.RData"))
}
