...
###########################################################################################
# 11162024 
# CRPS Measure for the binary_nsim_Basis SGLMM
rm(list=ls())
...
# for(phiselect in 2:length(phiset) ){
  ...
  for (sim in 1:n_simulations) {
    ...
    file_name3 <- paste0("../../02MCMC/Poisson/Basis_C_phi", phiselect, "_Gen25k_", sim, "_MCMC.RData")
    file_name4 <- paste0("../../03INFVB_ParallelComputing/Poisson/Basis_C_phi", phiselect, "_Gen25k_", sim, "_VBssqParallel.RData")
    file_name5 <- paste0("../../07INFVB_SA/Poisson/SA_Basis_C_phi", phiselect, "_Gen25k_", sim, "_VBssqParallel.RData")
    file_name6 <- paste0("../../08HybridMFVB_SA/Poisson/SA_Basis_C_phi", phiselect, "_Gen25k_", sim, "_HMFVB.RData")    
    file_name7 <- paste0("../../06INLA/Poisson/Basis_C_phi", phiselect, "_Gen25k_", sim, "_INLA.RData")    
    load(file_name1)
    load(file_name2)
    load(file_name3)
    load(file_name4)
  ...
  save(crpsTotal_MCMC_mean_vector, crpsTotal_INLA_mean_vector, 
       ...
       file = paste0("Basis_C_phi",phiselect,"_Gen25k_CRPS.RData"))
}
