...
# for(phiselect in 4:length(phiset) ){
  ...
  for (sim in 1:n_simulations) {
    ...
    file_name5 <- paste0("../../07INFVB_SA/Binary/SA_Basis_B_phi", phiselect, "_Gen25k_", sim, "_VBssqParallel.RData")
    file_name6 <- paste0("../../08HybridMFVB_SA/Binary/SA_Basis_B_phi", phiselect, "_Gen25k_", sim, "_HMFVB.RData")    
    file_name7 <- paste0("../../06INLA/Binary/Basis_B_phi", phiselect, "_Gen25k_", sim, "_INLA.RData")    
    load(file_name1)
    load(file_name2)
    load(file_name3)
    load(file_name4)
    ...
    ################################################################################
    ################################################################################
    # True Value in Credible Interval (3) HMFVB method 
    ################################################################################
    ################################################################################
    #For training Data
  ...
  save(TVCItrain_MCMC_mean_vector, TVCItrain_INLA_mean_vector, 
       ...
       file = paste0("Basis_B_phi",phiselect,"_Gen25k_TVCI.RData"))
}
